import logo from "../assets/digital-money.png";

type Session = Record<string, any>;

type NavbarProps = {
  login: () => void; // Assuming login is a function with no arguments and no return value
  logout: () => void; // Assuming login is a function with no arguments and no return value
  session: Session | undefined;
};
export default function Navbar({ login, logout, session }: NavbarProps) {
  return (
    <header className="d-flex flex-wrap align-items-center justify-content-center justify-content-md-between py-3 mb-4 border-bottom">
      <div className="col-md-3 mb-2 mb-md-0">
        <a
          href="/"
          className="d-inline-flex link-body-emphasis text-decoration-none"
        >
          <img
            className="bi"
            width="40"
            height="32"
            role="img"
            aria-label="logo"
            src={logo}
          ></img>
          {/* CPU 4 SALE */}
        </a>
      </div>

      <ul className="nav col-12 col-md-auto mb-2 justify-content-center mb-md-0">
        <li>
          <a href="/" className="nav-link px-2 link-secondary">
            Home
          </a>
        </li>
        <li>
          <a href="#features" className="nav-link px-2">
            Features
          </a>
        </li>
        <li>
          <a href="#faqs" className="nav-link px-2">
            FAQ
          </a>
        </li>
        <li>
          <a href="#about" className="nav-link px-2">
            About
          </a>
        </li>
      </ul>

      <div className="col-md-3 text-end">
        {session ? (
          // Render something when session is defined
          <button
            type="button"
            // className="btn btn-outline-primary me-2"
            className="btn btn-outline-danger"
            onClick={logout}
          >
            Logout
          </button>
        ) : (
          // Render something else when session is undefined
          <button
            type="button"
            // className="btn btn-outline-primary me-2"
            className="btn btn-primary"
            onClick={login}
          >
            Login
          </button>
        )}
      </div>
    </header>
  );
}
